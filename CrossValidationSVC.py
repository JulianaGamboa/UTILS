...
frutis=pd.read_csv("Data/1.DataFR_CT.csv")
...
algoritmo = SVC (gamma='scale', break_ties="True")
...
def test_seed(seed):
    ...
    results_SVC.append([score, seed])
for i in range(100):
    test_seed(i)    
results_SVC

results_sorted = sorted(results_SVC, key=lambda x: x[0], reverse=False)
...
modelo_ols = SVC(gamma='scale')
X = X
y = Tipo_encoded

results_cross_val_SVC = cross_val_score(
...
for i in range(2,13):
    ...
    resultados_cv_SVC.append(cv_rmse)

resultados_cv_SVC

import matplotlib.pyplot as plt
plt.plot(resultados_cv_SVC)
plt.xlabel("k-fold", fontsize = 10)
plt.ylabel("RMSE", fontsize = 10)
    
from sklearn.model_selection import cross_validate
scoring = {"mae": "neg_mean_absolute_error", "rmse": rmse_cross_val}
estimator = modelo_ols
scores_SVC = cross_validate(estimator, X,
                        y, scoring=scoring,
                         cv=13, return_train_score=True)
pd.DataFrame(scores_SVC).mean()
